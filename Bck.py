...
class Bck_out:
    ...
    def _get_fcurves(action):
        """Blender 5.0 compat: get fcurves from layered action or legacy action"""
    ...
    def dump_action(self, action, pose):
        loop_type_val = getattr(action, "bck_loop_type", 0)
        if isinstance(loop_type_val, str):
            self.loopType = LoopType[loop_type_val]
        else:
            self.loopType = LoopType(loop_type_val)
        self.maxframe = int(action.frame_range[1] - action.frame_range[0])

        all_fcurves = self._get_fcurves(action)
        ...
        for b in pose.bones:
            ...
            # Compute local matrix in Blender space (for translation)
            local_matrix = parent_mtx.inverted() @ b.bone.matrix_local
            local_rotation = local_matrix.to_quaternion()

            # Read the GC rest-pose euler angles stored during import as custom properties.
            # These are the exact JNT1 values — no coordinate conversion needed.
            gc_rx = b.bone.get("gc_rest_rx", 0.0)
            ...
            trans_fcurves = [fcu for fcu in all_fcurves if fcu.data_path.startswith(fcurve_path + ".location")]
            rot_fcurves = [fcu for fcu in all_fcurves if fcu.data_path.startswith(fcurve_path + ".rotation_euler")]
    ...
    def get_track_keyframe(self, x_curve, y_curve, z_curve, frame, is_scale=False):
        ...
        if x_co is not None and y_co is not None and z_co is not None:
            # Extract tangent slopes from Blender handles: slope = dy/dx
            def _slope(handle, co):
                dx = handle[0] - co[0]
                if abs(dx) < 1e-10:
                    return 0.0
                return (handle[1] - co[1]) / dx

            x_tangent_left = _slope(x_handle_left, x_co)
            x_tangent_right = _slope(x_handle_right, x_co)
            y_tangent_left = _slope(y_handle_left, y_co)
            y_tangent_right = _slope(y_handle_right, y_co)
            z_tangent_left = _slope(z_handle_left, z_co)
            z_tangent_right = _slope(z_handle_right, z_co)

            if is_scale:
                # Scale: swap Y<->Z, no negation (inverse of import's vct.z,vct.y = vct.y,vct.z)
                ...
                # Inverse of import's axis swap: import does vct.z,vct.y = vct.y,-vct.z
                # So: blender_y = gc_y, blender_z = -gc_z
                # Inverse: gc_y = blender_y, gc_z = -blender_z
                # In tuple: (x, -z, y) — but fcurves are indexed as Blender [x,y,z]
                # Import maps GC(x,y,z) → Blender(x, -z, y) stored in XZY mode
                # Export needs Blender(x,y,z) in XZY fcurves → GC(x,y,z)
                # Blender XZY fcurve[0]=X, fcurve[1]=Z', fcurve[2]=Y'
                # where Z' = -gc_z, Y' = gc_y
                # So: gc_x = fcurve_x, gc_y = fcurve_z, gc_z = -fcurve_y
                # Import stored: gc_x→fcurve[0], -gc_z→fcurve[1], gc_y→fcurve[2]
                # So: x_co=gc_x, y_co=-gc_z, z_co=gc_y
                # Reverse: gc_x=x_co, gc_y=z_co, gc_z=-y_co
                keyframe_value = (x_co[1], z_co[1], -y_co[1])
                tangent_left = (x_tangent_left, z_tangent_left, -y_tangent_left)
                tangent_right = (x_tangent_right, z_tangent_right, -y_tangent_right)
        else:
    ...
    def process_rotation_track(self, curves, frame, anim, gc_rest_quat):
        x_track = None
        y_track = None
        z_track = None
        ...
        # BCK stores ABSOLUTE rotations. Import did rotate(inv_rest_quat) to get delta.
        # Export reverses: rotate(rest_quat) to get back absolute.
        # gc_rest_quat is the GC-space rest pose (not Blender-space).
        rot_euler = mathutils.Euler(value, 'XYZ')
        rot_euler.rotate(gc_rest_quat)

        tgL_nearby = mathutils.Euler((value[0] + EPSILON * handle_left[0],
                                       value[1] + EPSILON * handle_left[1],
                                       value[2] + EPSILON * handle_left[2]), 'XYZ')
        tgR_nearby = mathutils.Euler((value[0] + EPSILON * handle_right[0],
                                       value[1] + EPSILON * handle_right[1],
                                       value[2] + EPSILON * handle_right[2]), 'XYZ')
        tgL_nearby.rotate(gc_rest_quat)
        tgR_nearby.rotate(gc_rest_quat)

        x_bck_key = BckKey()
        x_bck_key.time = frame
        x_bck_key.tangentL = (tgL_nearby[0] - rot_euler[0]) / EPSILON
        x_bck_key.tangentR = (tgR_nearby[0] - rot_euler[0]) / EPSILON
        x_bck_key.value = self.correct_rotation(rot_euler[0])
        anim.rotationsX.append(x_bck_key)

        y_bck_key = BckKey()
        y_bck_key.time = frame
        y_bck_key.tangentL = (tgL_nearby[1] - rot_euler[1]) / EPSILON
        y_bck_key.tangentR = (tgR_nearby[1] - rot_euler[1]) / EPSILON
        y_bck_key.value = self.correct_rotation(rot_euler[1])
        anim.rotationsY.append(y_bck_key)

        z_bck_key = BckKey()
        z_bck_key.time = frame
        z_bck_key.tangentL = (tgL_nearby[2] - rot_euler[2]) / EPSILON
        z_bck_key.tangentR = (tgR_nearby[2] - rot_euler[2]) / EPSILON
