...
class ExportBck(Operator, ExportHelper):
    ...
    def execute(self, context):
        ...
        # Try dropdown first, fall back to active object
        anim_armature = bpy.data.objects.get(context.scene.anim_export_armatures)
        if anim_armature is None:
            anim_armature = context.active_object
        if anim_armature is None or anim_armature.type != 'ARMATURE':
            self.report({'ERROR'}, "No armature selected")
            return {'CANCELLED'}

        # Try dropdown action, fall back to active action on the armature
        out_action = bpy.data.actions.get(context.scene.anim_export_actions)
        if out_action is None and anim_armature.animation_data:
            out_action = anim_armature.animation_data.action
